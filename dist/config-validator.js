export class ConfigValidationService {
    async validateConfig(config, filetype = 'lua') {
        const issues = [];
        const suggestions = [];
        try {
            if (filetype === 'lua') {
    ...
    async generateConfig(useCase, description) {
        const configs = {
    ...
    validateLuaConfig(config, issues, suggestions) {
    ...
    validateVimConfig(config, issues, suggestions) {
        const lines = config.split('\n');
        lines.forEach((line, index) => {
            const lineNum = index + 1;
            const trimmed = line.trim();
    ...
    formatValidationResult(issues, suggestions) {
        let result = '';
...
autocmd("BufReadPost", {
  ...
  callback = function()
    ...
    generatePluginSetup(description) {
...
lspconfig.lua_ls.setup({
  ...
  settings = {
    ...
    generateColorscheme(description) {
...
vim.cmd.colorscheme("catppuccin")`;
    ...
    generateStatusline(description) {
...
keymap("n", "<leader>fh", "<cmd>Telescope help_tags<cr>", { desc = "Help tags" })`;
    ...
    generateTreesitter(description) {
