export class NvimDocumentationService {
  private readonly NVIM_DOCS_BASE = 'https://neovim.io/doc/user/';
  private readonly NVIM_API_BASE = 'https://neovim.io/doc/api/';
  private readonly docCache = new Map<string, any>();

  async searchDocs(query: string, type: string = 'all') {
    const results: any[] = [];
...
- Setup servers in lua/lsp/ directory`,
  ...
  private async searchFunctions(query: string) {
    const functions = [
  ...
  private formatSearchResults(results: any[], query: string): string {
    ...
    results.forEach((result, index) => {
      ...
      if (result.type) output += `   Type: ${result.type}\n`;
      if (result.syntax) output += `   Syntax: ${result.syntax}\n`;
      if (result.params) output += `   Parameters: ${result.params}\n`;
  ...
  private async fetchHelpTopic(topic: string): Promise<string> {
